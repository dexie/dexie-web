...
// Read credentials from dexie-cloud.key (generated by `dexie-cloud connect`)
...
The SDK is designed for **server-to-server access**. Use `client_credentials` authentication
with the `client_id` and `client_secret` from your `dexie-cloud.key` file.

The `dexie-cloud.key` file is created when you run `dexie-cloud connect` (see [CLI docs](/cloud/docs/cli)):

```bash
npx dexie-cloud connect https://xxxxxxxx.dexie.cloud
```

This creates `dexie-cloud.key` in your project directory with your credentials.
...
The SDK uses [TSON](/cloud/docs/tson) (Typed JSON) for all data serialization. This means rich JavaScript types survive round-trips:
